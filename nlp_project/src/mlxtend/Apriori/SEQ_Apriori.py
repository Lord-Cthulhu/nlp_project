    ...
    try:      
        ...
        for idoc in corp:
            ...
            for word in doc:
                ...
                pos_dep.append(word.pos_+'|'+word.dep_)
                #print(word.pos_, word.tag_, word.dep_, word, sep=",")
           
            pos_val = ','.join(pos)
            dep_val = ','.join(dep)
            pos_dep_val = ','.join(pos_dep)
            ...
            df_seq = df_seq.append(data_mod, ignore_index=True)
            #df_seq = df_seq.concat(data_mod, ignore_index=True)
        df_seq = df_seq.astype(str)                        
        df_seq["pos"]= df_seq["pos"].str.get(0)
        df_seq["dep"]= df_seq["dep"].str.get(0)
        #df_seq["pos_dep"]= df_seq["pos_dep"].str.get(0)

        if posx == 'pos':
            df_seq = pd.concat([df_seq,df_seq.pos.str.split(',',expand=True).add_prefix('pos_')],axis=1)
        if posx == 'dep':
            df_seq = pd.concat([df_seq,df_seq.dep.str.split(',',expand=True).add_prefix('dep_')],axis=1) 
        if posx == 'pos_dep':
            df_seq = pd.concat([df_seq,df_seq.pos_dep.str.split(',',expand=True).add_prefix('pos_dep_')],axis=1)  
        if posx == 'all':
        #df_seq = pd.concat([df_seq,df_seq.words.str.split(',',expand=True).add_prefix('word_')],axis=1)
            df_seq = pd.concat([df_seq,df_seq.pos.str.split(',',expand=True).add_prefix('pos_')],axis=1)
            df_seq = pd.concat([df_seq,df_seq.dep.str.split(',',expand=True).add_prefix('dep_')],axis=1) 
            df_seq = pd.concat([df_seq,df_seq.pos_dep.str.split(',',expand=True).add_prefix('pos_dep_')],axis=1)
        print(df_seq)
        ...
        print("doc_pos_seq error")
        print(e)
        #traceback.print(file=sys.stdout)
...
# df_seq = doc_pos_seq(corp, 'pos_dep')
    
# print(df_seq)

# df_seq.to_excel('out/gads_data_por_sqdone_no_dup.xlsx')


# df = pd.DataFrame()

# df['0'] = df_seq['pos_dep_0']
# df['1'] = df_seq['pos_dep_1']
# df['2'] = df_seq['pos_dep_2']
# df['3'] = df_seq['pos_dep_3']
# df['4'] = df_seq['pos_dep_4']
# df['5'] = df_seq['pos_dep_5']
# df['6'] = df_seq['pos_dep_6']
# df['7'] = df_seq['pos_dep_7']
# df['8'] = df_seq['pos_dep_8']
# df['9'] = df_seq['pos_dep_9']
...
# encoded_vals[0]
# ohe_df2 = pd.DataFrame(encoded_vals)
# print(ohe_df2)

# freq_items = apriori(ohe_df2, min_support=0.01, use_colnames=True, verbose=1)
# print(freq_items.head(5))
# freq_items.to_excel('out/pos_freq_no_dup.xlsx')

# rules = association_rules(freq_items, metric="confidence", min_threshold=0.2)
# print(rules.head(5))
# rules.to_excel('out/pos_rules_no_dup.xlsx')
...
rules2.to_excel('C:/Users/phili/OneDrive/Bureau/Google_Demo/Google_Demo/Prototype/data/token_rules_new8_no_dup_trans.xlsx')
freq_items2.to_excel('C:/Users/phili/OneDrive/Bureau/Google_Demo/Google_Demo/Prototype/data/token_freq_new8_no_dup_trans.xlsx')
